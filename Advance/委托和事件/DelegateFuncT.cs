...
public class DelegateFuncT : MyBasePrintClass
{
    ...
    public void Test1()
    {
        Action action1 = delegate() { WriteLine("这是一个无参无返回的匿名函数"); };

        Action<int> action2 = delegate(int value) { WriteLine($"这是一个有参无返回的匿名函数，参数: {value}"); };
        ...
        //delegate 可以省略
        //这种即为 lambda 表达式
        Action action1L = () => { WriteLine("这是一个无参无返回的匿名函数"); };
        //参数类型都可以省略，只需与委托或事件容器一致即可
        Action<int> action2L = (int value) => { WriteLine($"这是一个有参无返回的匿名函数，参数: {value}"); };
    }
    ...
    public void Test2()
    {
        ...
        {
            WriteLine("这是一个无参有返回的匿名函数");
            return "Hello";
        };

        Func<int, string> action2 = delegate(int value)
        {
            WriteLine($"这是一个有参有返回的匿名函数，参数: {value}");
            return "World";
        };
    ...
    public Action DoSomething()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            action += () => { WriteLine($"i: {i}, value: {value}"); };
        }
