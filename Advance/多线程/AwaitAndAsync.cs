...
public class AwaitAndAsync : MyBasePrintClass
{
    ...
    public void Test1()
    {
        output.WriteLine("当前打印主线程ID：{0}", Thread.CurrentThread.ManagedThreadId);
        TestAsync();
        output.WriteLine("再次打印主线程ID：{0}", Thread.CurrentThread.ManagedThreadId);
        Thread.Sleep(2000);
    }
    ...
    {
        //当前还在主线程中执行
        output.WriteLine("打印 async 方法线程ID：{0}", Thread.CurrentThread.ManagedThreadId);
        //检测到 await 关键字后，当前函数会被挂机，继续返回执行 调用处 函数剩下的
        //会新开一个线程 执行 await 修饰的任务
        //await 中不能访问 unity 对象
        await Task.Run(() =>
        {
            output.WriteLine("await 任务线程ID：{0}", Thread.CurrentThread.ManagedThreadId);
            Thread.Sleep(1500);
            output.WriteLine("await 任务执行完毕");
        });
        //await 修饰的线程池任务执行完毕后，剩下的任务才会执行
        output.WriteLine("再次打印 async 方法线程ID：{0}", Thread.CurrentThread.ManagedThreadId);
    }
}